"""Experiment script for the coupled Duffing oscillator: data generation, cross-validation, training, and plotting."""
...
def makeAllData():
    ...
    omega             = np.c_[X.flatten(), Y.flatten(), Z.flatten(), W.flatten()].T
    cubeBound         = np.array([[-1, 1], [-1, 1], [-1, 1], [-1, 1]])
...
def computeErrorsControl():
    ...
    newkernel         = kernel.QuartMaternProduct(0.07)
...
def plotResult():
    ...
    with plt.rc_context(rc):
        ...
        ax.set_yscale('log')
        ax.set_title('Coupled Duffing Oscillator (\\texttt{CoupledDuffing})', pad=10)
